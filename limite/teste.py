...
def make_window(theme):
    ...
    layout = [[sg.Text('Sistema de agendamento de vacinações', size=(38, 1), justification='center', font=("Helvetica", 16), relief=sg.RELIEF_RIDGE, k='-TEXT HEADING-', enable_events=True)]]
    layout +=[[sg.TabGroup([[  sg.Tab('Relatório', input_layout),
                               sg.Tab('Agendamento', asthetic_layout),
                               sg.Tab('Vacinas', graphing_layout),
                               sg.Tab('Enfermeiros', specalty_layout),
                               sg.Tab('Pacientes', theme_layout),
                               sg.Tab('Output', logging_layout)]], key='-TAB GROUP-')]]
              
    return sg.Window('Posto de Vacinação', layout, right_click_menu=right_click_menu_def)
